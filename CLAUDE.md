...
**Required by Claude Code (do not remove), installed before Claude Code itself:**

| Tool              | Why it's needed                                                                                    |
| ------------------ | --------------------------------------------------------------------------------------------------- |
| `ca-certificates` | TLS trust store - required for secure HTTPS connections (Claude Code installer, API calls)          |
| `curl`             | HTTP client - required to run the Claude Code install script                                        |
...
**Recommended defaults - enable specific features, safe to add or remove for your project:**
...
| `npmrc`               | npm supply-chain hardening policy (ignore-scripts, min-release-age, etc); mounted read-only as the agent user's npm config at `/home/agent/.npmrc` |
